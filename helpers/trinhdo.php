...
class Trinhdo {
    ...
    public static function add($post = array()) {
        $objTable = JTable::getInstance('TienluongTrinhdo', 'Table');
    ...
    public static function delete() {
        ...
        if (count($arrId) > 0) {
            $record = JTable::getInstance('TienluongTrinhdo', 'Table');
            foreach ($arrId as $id) {
                if (!$record->delete($id)) {
    ...
    public static function getAll() {
        $db = JFactory::getDbo();
        $query = "SELECT * FROM #__trinhdo ORDER BY id DESC";
        $db->setQuery($query);
    ...
    public static function getInfoById($id) {
        $db = JFactory::getDbo();
        $query = "SELECT * FROM #__trinhdo WHERE id = {$id}";
        $db->setQuery($query);
