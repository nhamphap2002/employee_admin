...
class Quatrinhdt {
    ...
    public static function add($post = array()) {
        $objTable = JTable::getInstance('TienluongQuatrinhdt', 'Table');
    ...
    public static function delete() {
        ...
        if (count($arrId) > 0) {
            $record = JTable::getInstance('TienluongQuatrinhdt', 'Table');
            foreach ($arrId as $id) {
                if (!$record->delete($id)) {
    ...
    public static function getAll() {
        $db = JFactory::getDbo();
        $query = "SELECT * FROM #__quatrinhdt ORDER BY id DESC";
        $db->setQuery($query);
    ...
    public static function getInfoById($id) {
        $db = JFactory::getDbo();
        $query = "SELECT * FROM #__quatrinhdt WHERE id = {$id}";
        $db->setQuery($query);
