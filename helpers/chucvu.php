...
class Chucvu {
    ...
    public static function add($post = array()) {
        $objTable = JTable::getInstance('TienluongChucvu', 'Table');
    ...
    public static function delete() {
        ...
        if (count($arrId) > 0) {
            $record = JTable::getInstance('TienluongChucvu', 'Table');
            foreach ($arrId as $id) {
                if (!$record->delete($id)) {
    ...
    public static function getAll() {
        $db = JFactory::getDbo();
        $query = "SELECT * FROM #__chucvu ORDER BY id DESC";
        $db->setQuery($query);
    ...
    public static function getInfoById($id) {
        $db = JFactory::getDbo();
        $query = "SELECT * FROM #__chucvu WHERE id = {$id}";
        $db->setQuery($query);
